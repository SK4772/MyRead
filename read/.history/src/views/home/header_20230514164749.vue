...
<template>
  ...
  <header>
    <el-dialog :visible.sync="visible2">
      <el-descriptions title="单词查找结果" column=1>
        <el-descriptions-item label="单词">{{word.componet}}</el-descriptions-item>
        <el-descriptions-item label="意思">{{word.meaning}}</el-descriptions-item>
        <el-descriptions-item label="英式音标">{{word.uKPhonetic}}</el-descriptions-item>
        <el-descriptions-item label="美式音标">{{word.usPhonetic}}</el-descriptions-item>
        <el-descriptions-item label="单词词组">{{word.wordGroupMeaning1}}</el-descriptions-item>
        <el-descriptions-item label="词组翻译">{{word.explain}}</el-descriptions-item>
      </el-descriptions>
    </el-dialog>
    <el-dialog
      title="个人中心"
...
// ^[A-Za-z0-9\u4e00-\u9fa5]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$
  ...
  methods: {
    ...
    // 搜索框绑定事件
    search() {
      this.$axios.post("word/queryword",{
        componet:this.componet
      }).then((res)=>{
        this.word=res.data;
        this.visible2=true;
