      ...
      this.$axios
        ...
        .then((response) => {
          ...
          let s1=Math.floor(Math.random()*4);
           ...
           let s3=Math.floor(Math.random()*4);
          this.tableData[s1].name=response.data[0];
          this.tableData[s2].name=response.data[1];
          this.tableData[s3].name=response.data[2];
        });
...
————————————————
版权声明：本文为CSDN博主「小呀小萝卜儿」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/weixin_48850734/article/details/129747994
    getWortList() {
      this.getSelectMeaning();
      this.$axios
        .post("/word/getNoWord", {
          userid: this.$route.params.id,
        })
