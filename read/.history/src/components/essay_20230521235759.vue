<template>
  <div class="page">
    ...
    <div class="note_panel">
      ...
      <el-button class="notebutton" @click="updateNote()"> 确认 </el-button>
    </div>
    <el-button class="nextbutton"  @click="next()"> 下一篇</el-button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // },
    getEssayList() {
      this.$axios.get("/essay/getAllEssay", {}).then((response) => {
        console.log(response);
        if(this.i<response.data.length){
          this.essay = response.data[this.i];
        this.url = this.essay.essayPath;
        this.userid = this.$route.params.id;
        this.essayid = this.essay.essayid;
    ...
    updateNote() {
      this.$axios
        .post("/admin/note/updateNoteUser", {
          userid: this.userid,
    ...
    getnode() {
      this.$axios
        ...
        .then((response) => {
          console.log(response);
          this.node = response.data[0].node;
        })
        .catch(function (error) {
          console.log(error);
        });
