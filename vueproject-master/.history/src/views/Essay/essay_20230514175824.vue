      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            type="text"
            size="small"
            @click="deleteHandle(scope.row.essayid, scope.row.componet)"
          >
            删除
          </el-button>
        </template>
...
let formData = new FormData();
            console.log("fgjdkgbnv ")
        formData.append("file", fileList[0].raw);
          this.$axios.post("/test",formData, {
        "Content-Type": "multipart/form-data"
      }).then((res)=>{
          this.filepath=res.data.message.data;
            console.log(res)
    ...
    dataFormSubmit() {
      if (this.dataForm.essayid) {
        this.$refs["dataForm"].validate((valid) => {
          if (valid) {
            this.$axios
              .post("/admin/essay/updataEssay", this.dataForm)
              .then((res) => {
                if (res.data == true) {
                  this.$message({
                    message: "操作成功",
        ...
        from.append("essaypath",this.filePath);
        from.append("title",this.dataForm.title);
        from.append("classify",this.dataForm.classify);
        this.$refs["dataForm"].validate((valid) => {
          if (valid) {
            this.$axios
              .post("/admin/essay/addEssay", {
                "essaypath":this.filePath,
    ...
    queryByComponet() {
      this.$axios
        .post("/admin/essay/queryByID", {
          essayid: this.wordInputContent,
        })
        .then((response) => {
